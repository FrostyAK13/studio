            ...
            <Card className="border-none shadow-sm bg-card overflow-hidden relative rounded-2xl border border-border">
                ...
                <CardHeader className="text-center pt-8 px-6">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <div>
                            <CardTitle className="text-lg sm:text-xl font-black uppercase tracking-widest text-foreground leading-tight">MARKET SCANNER</CardTitle>
                        </div>
                    </div>
                </CardHeader>
                    ...
                    <div className="flex justify-center">
                        <Button 
                            ...
                            className={cn(
                                ...
                                status === 'scanning' ? "bg-muted text-muted-foreground cursor-not-allowed" : "bg-primary text-primary-foreground hover:bg-primary/90"
                            )}
                        >
                            <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                            {status === 'scanning' ? (
                                <>
                                    <Loader2 className="mr-2 h-4 sm:h-5 animate-spin" />
                                    SYNCING MARKET VECTORS...
                                </>
                            ) : (
                                <>
                                    <Search className="mr-2 h-4 sm:h-5" />
                    ...
                    <div className="relative min-h-[200px] flex flex-col items-center justify-center">
                        <AnimatePresence mode="wait">
                            {status === 'scanning' && (
                                ...
                                >
                                    ...
                                    <div className="text-center space-y-2">
                                        ...
                                        <div className="w-full bg-muted h-2 rounded-full overflow-hidden border border-border shadow-inner">
                                            <motion.div 
                            ...
                            {status === 'results' && result && (
                                ...
                                >
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4">
                                        <Card className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900 p-4 rounded-2xl relative group" >
                                            <div className="absolute top-2 right-3"><Crosshair className="h-3 w-3 text-emerald-600" /></div>
                                            <p className="text-[7px] font-black text-emerald-600 uppercase tracking-widest mb-1">MARKET</p>
                                            <p className="text-xs sm:text-sm font-black text-foreground leading-tight">{result.marketName}</p>
                                        </Card>
                                        ...
                                        <Card className="bg-primary/5 border border-primary/20 p-4 rounded-2xl relative">
                                            ...
                                            <p className="text-[7px] font-black text-primary uppercase tracking-widest mb-1">STRATEGY</p>
                                            <p className="text-sm sm:text-lg font-black text-foreground leading-tight">{result.strategy}</p>
                                        </Card>

                                        <Card className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900 p-4 rounded-2xl relative animate-in zoom-in-95 duration-700">
                                            <div className="absolute top-2 right-3"><Wallet className="h-3 w-3 text-blue-600" /></div>
                                            <p className="text-[7px] font-black text-blue-600 uppercase tracking-widest mb-1">LIVE PIVOT</p>
                                            <p className="text-sm sm:text-lg font-black text-foreground tabular-nums">
                                                {price === 0 ? (
                                        ...
                                        <Card className="bg-cyan-50 dark:bg-cyan-950/20 border border-cyan-200 dark:border-cyan-900 p-4 rounded-2xl relative">
                                            <div className="absolute top-2 right-3"><Target className="h-3 w-3 text-cyan-600" /></div>
                                            <p className="text-[7px] font-black text-cyan-600 uppercase tracking-widest mb-1">ENTRY SIGNAL</p>
                                            <div className="flex items-center gap-3">
                                                <div>
                                                    <p className="text-[6px] text-muted-foreground uppercase mb-0.5">TRIGGER</p>
                                                    <p className="text-base sm:text-xl font-black text-foreground">{result.triggerDigit}</p>
                                                </div>
                                                <div className="w-px h-6 bg-border" />
                                                <div>
                                                    <p className="text-[6px] text-muted-foreground uppercase mb-0.5">RECOVERY</p>
                                                    <p className="text-base sm:text-xl font-black text-foreground">{result.recoveryDigit}</p>
                                                </div>
                                            </div>
                                        </Card>

                                        <Card className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 p-4 rounded-2xl relative">
                                            <div className="absolute top-2 right-3"><BarChart3 className="h-3 w-3 text-amber-600" /></div>
                                            <p className="text-[7px] font-black text-amber-600 uppercase tracking-widest mb-1">STABILITY</p>
                                            <p className="text-base sm:text-xl font-black text-foreground leading-tight tabular-nums">{result.successRate}%</p>
                                    ...
                                    <Card className={cn(
                                        "p-6 sm:p-8 rounded-[2rem] border-2 transition-all duration-500 relative overflow-hidden",
                                        entryDetected 
                                            ...
                                            <div className="flex items-center gap-8">
                                                {entryPrice && (
                                                    <div className="text-center animate-in zoom-in duration-500">
                                                        ...
                                                        <p className="text-lg sm:text-2xl font-black text-foreground tabular-nums leading-none">{entryPrice.toFixed(decimalPlaces)}</p>
                                                    </div>
                                                )}
                                        ...
                                        {entryDetected && (
                                            <div className="mt-6 h-1.5 w-full bg-emerald-100 dark:bg-emerald-900 rounded-full overflow-hidden">
                                                <motion.div 
                                    ...
                                    <div className="p-5 sm:p-8 bg-muted/30 rounded-[2rem] border border-border space-y-3 shadow-inner">
                                        ...
                                        <p className="text-[11px] sm:text-sm font-medium text-foreground leading-relaxed italic">
                                            "{result.reasoning}"
                                        </p>
                                    </div>
                                </motion.div>
                            ...
                            {status === 'idle' && (
                                ...
                                >
                                    <div className="w-14 h-14 sm:w-16 rounded-full bg-muted border border-border flex items-center justify-center mx-auto opacity-20">
                                        <Orbit className="h-7 sm:h-8 text-muted-foreground" />
                                    </div>
                                    <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground">ENGINE STANDBY</p>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
