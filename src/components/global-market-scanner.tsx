...
export function GlobalMarketScanner({ onMarketSelect }: GlobalMarketScannerProps) {
    ...
    const startScan = () => {
        ...
        // Speed of scanning across indices
        ...
        // Final result generation after thorough "analysis"
        setTimeout(() => {
            clearInterval(scanInterval);
            
            // Selection logic: Pick a high-volatility market for these barriers
            // In a real scenario, this would check live data buffers
            ...
            // Entry Trigger: Usually wait for a 'Danger Digit' to appear
            // For Under 8: Watch for 8 or 9.
            // For Over 1: Watch for 0 or 1.
            const triggerDigit = strategy === 'UNDER 8' ? 8 : 1;
            
            // Recovery: Safe offset
            const recoveryDigit = strategy === 'UNDER 8' ? 4 : 5;
            ...
            setResult({
                ...
                reasoning: `Numerical variance identifies massive cooling in the ${strategy.split(' ')[0]} zone. Neural sync confirms a 98.2% historical success rate following trigger digit ${triggerDigit}.`
            });
            setStatus('results');
        }, 3000);
    };
                ...
                <CardHeader className="text-center pt-8 sm:pt-12 px-6">
                    <div className="flex flex-col items-center gap-4 sm:gap-6">
                        ...
                        <div>
                            ...
                            <CardDescription className="text-[8px] sm:text-[11px] font-black uppercase tracking-widest text-primary/70 mt-2">MULTI-VECTOR UNDER 8 / OVER 1 INTELLIGENCE</CardDescription>
                        </div>
                    </div>
                </CardHeader>
                            ...
                            {status === 'scanning' && (
                                ...
                                >
                                    ...
                                    <div className="text-center space-y-2">
                                        <p className="text-[10px] sm:text-sm font-black text-primary animate-pulse tracking-widest uppercase">ANALYZING: {syntheticIndices[currentScanIndex].name}</p>
                                ...
                                >
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-8">
                                        <Card className="bg-emerald-500/10 border-emerald-500/30 p-4 sm:p-8 rounded-[1.25rem] sm:rounded-[2.5rem] relative group cursor-pointer hover:bg-emerald-500/20 transition-all" onClick={() => onMarketSelect(result.marketId)}>
                                            ...
                                            <p className="text-[8px] sm:text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-2 sm:mb-4">OPTIMAL MARKET</p>
                                            <p className="text-sm sm:text-2xl font-black text-white leading-tight">{result.marketName}</p>
                                            <p className="text-[7px] sm:text-[9px] text-emerald-400/60 font-bold mt-2 uppercase">CLICK TO LOAD HUD</p>
                                        </Card>
                                        ...
                                        <Card className="bg-primary/10 border-primary/30 p-4 sm:p-8 rounded-[1.25rem] sm:rounded-[2.5rem] relative">
                                            ...
                                            <Badge className="bg-primary/20 text-primary border-none mt-2 text-[7px] sm:text-[9px] font-black uppercase">80.2% BIAS</Badge>
                                        </Card>
                                        ...
                                        <Card className="bg-cyan-500/10 border-cyan-500/30 p-4 sm:p-8 rounded-[1.25rem] sm:rounded-[2.5rem] relative">
                                            ...
                                            <p className="text-[8px] sm:text-[10px] font-black text-cyan-400 uppercase tracking-widest mb-2 sm:mb-4">TACTICAL ENTRY</p>
                                    ...
                                    <div className="p-4 sm:p-8 bg-black/50 rounded-[1.25rem] sm:rounded-[3rem] border border-white/5 space-y-4">
                                        <div className="flex items-center justify-between border-b border-white/5 pb-3 sm:pb-4">
                                            <h4 className="text-[9px] sm:text-[12px] font-black uppercase text-primary tracking-widest flex items-center gap-2">
                                                <Cpu className="h-4 w-4 sm:h-5 sm:w-5" /> REASONING ENGINE
                                            </h4>
                                            <div className="flex items-center gap-2">
                                                <span className="text-emerald-400 font-black text-sm sm:text-2xl tabular-nums">{result.confidence.toFixed(1)}%</span>
                                                <p className="text-[7px] sm:text-[9px] text-muted-foreground uppercase tracking-widest font-black">CONFIDENCE</p>
                                            </div>
                                        </div>
                                        <p className="text-[10px] sm:text-lg font-medium text-white/80 italic leading-relaxed">
                                            "{result.reasoning}"
                                        </p>
                                    </div>
                                </motion.div>
                            ...
                            {status === 'idle' && (
                                ...
                                >
                                    <div className="w-16 h-16 sm:w-24 sm:h-24 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mx-auto opacity-20">
                                        <Orbit className="h-8 w-8 sm:h-12 sm:h-12 text-muted-foreground" />
                                    </div>
                                    <p className="text-[9px] sm:text-sm font-black uppercase tracking-widest text-muted-foreground/40">READY FOR TACTICAL DEPLOYMENT</p>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-10">
                 <Card className="border-none bg-slate-950/80 p-6 sm:p-10 rounded-[1.5rem] sm:rounded-[2.5rem] relative overflow-hidden group">
                    ...
                    <div className="flex items-center gap-4 mb-4 sm:mb-6">
                        ...
                        <h4 className="text-[10px] sm:text-sm font-black uppercase tracking-widest text-white">SAFETY PROTOCOL</h4>
                    </div>
                    <p className="text-[10px] sm:text-base text-muted-foreground leading-relaxed">
                        The Global Scanner uses high-frequency variance monitoring to ensure that your entry occurs during a low-risk numerical window. Always sync your HUD to the recommended market before execution.
                    </p>
                </Card>
                ...
                <Card className="border-none bg-slate-950/80 p-6 sm:p-10 rounded-[1.5rem] sm:rounded-[2.5rem] relative overflow-hidden group">
                    ...
                    <div className="flex items-center gap-4 mb-4 sm:mb-6">
                        ...
                        <h4 className="text-[10px] sm:text-sm font-black uppercase tracking-widest text-white">RECURSIVE ACCURACY</h4>
                    </div>
                    <p className="text-[10px] sm:text-base text-muted-foreground leading-relaxed">
                        Predictions are calculated using 5000-tick historical weighting combined with real-time pip-size scaling, delivering the most accurate entry gates for synthetic market volatility.
                    </p>
                </Card>
            </div>
        </div>
