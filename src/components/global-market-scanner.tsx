...
export function GlobalMarketScanner({ onMarketSelect, lastDigitTicks = [] }: GlobalMarketScannerProps) {
    ...
    const startScan = () => {
        ...
        setTimeout(() => {
            ...
            setResult({
                ...
                reasoning: `VOLATILITY SYNC: Identified high-stability cycle in ${bestIndex.name}. ${strategy} engine optimized for immediate engagement. Awaiting Trend Trigger Digit ${triggerDigit} for flawless execution. Recovery Pivot locked at Digit ${recoveryDigit}.`
            });
            setStatus('results');
        }, 3000);
    };
    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700 pb-24 max-w-5xl mx-auto">
            <Card className="border-none shadow-2xl bg-slate-900/40 backdrop-blur-3xl overflow-hidden relative rounded-[1.5rem] sm:rounded-[2.5rem]">
                ...
                <CardHeader className="text-center pt-10 px-6">
                    <div className="flex flex-col items-center gap-6">
                        <div className="w-20 h-20 rounded-[1.5rem] sm:rounded-[2rem] bg-primary/20 flex items-center justify-center border border-primary/30 shadow-[0_0_20px_rgba(var(--primary),0.3)]">
                            <Network className="h-10 w-10 text-primary animate-pulse" />
                        </div>
                        <div>
                            <CardTitle className="text-xl sm:text-2xl font-black uppercase tracking-widest text-white leading-tight">GLOBAL TACTICAL SCANNER</CardTitle>
                ...
                <CardContent className="p-6 sm:p-12 space-y-12">
                    <div className="flex justify-center">
                        <Button 
                            ...
                            className={cn(
                                "h-16 sm:h-20 px-8 sm:px-12 rounded-full font-black text-[10px] sm:text-sm uppercase tracking-[0.2em] shadow-2xl transition-all active:scale-95 group relative overflow-hidden",
                        ...
                        >
                            ...
                            {status === 'scanning' ? (
                                <>
                                    <Loader2 className="mr-3 sm:mr-4 h-5 w-5 sm:h-7 sm:w-7 animate-spin" />
                                    MAPPING VOLATILITY FLOW...
                                </>
                            ) : (
                                <>
                                    <Search className="mr-3 sm:mr-4 h-5 w-5 sm:h-7 sm:w-7" />
                                    ...
                                    <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                                        {volatilityIndices.map((m, idx) => (
                                            <div 
                                                key={m.id} 
                                                className={cn(
                                                    "px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border text-[8px] sm:text-[9px] font-black uppercase transition-all duration-100",
                            ...
                            {status === 'results' && result && (
                                ...
                                >
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
                                        <Card className="bg-emerald-500/10 border border-emerald-500/30 p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] relative group cursor-pointer hover:bg-emerald-500/20 transition-all" onClick={() => onMarketSelect(result.marketId)}>
                                            ...
                                            <p className="text-lg sm:text-xl font-black text-white leading-tight">{result.marketName}</p>
                                            <p className="text-[8px] text-emerald-400/60 font-bold mt-2 uppercase tracking-tighter">TAP TO ENGAGE HUD</p>
                                        </Card>

                                        <Card className="bg-primary/10 border border-primary/30 p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] relative">
                                        ...
                                        <Card className="bg-cyan-500/10 border border-cyan-500/30 p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] relative">
                                            ...
                                            <div className="flex items-center gap-4 sm:gap-6">
                                                <div>
                                                    ...
                                                    <p className="text-2xl sm:text-3xl font-black text-white">{result.triggerDigit}</p>
                                                </div>
                                                <div className="w-px h-8 sm:h-10 bg-white/10" />
                                                <div>
                                                    <p className="text-[8px] text-muted-foreground uppercase mb-1">RECOVERY</p>
                                                    <p className="text-2xl sm:text-3xl font-black text-white">{result.recoveryDigit}</p>
                                                </div>
                                            </div>
                                        </Card>
                                    </div>

                                    <div className="p-5 sm:p-8 bg-black/50 rounded-[1.5rem] sm:rounded-[2.5rem] border border-white/5 space-y-4">
                                        <div className="flex items-center justify-between border-b border-white/5 pb-3">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className="text-base sm:text-xl text-emerald-400 font-black tabular-nums">{result.confidence.toFixed(1)}%</span>
                                                <p className="text-[8px] text-muted-foreground uppercase tracking-widest font-black">ACCURACY</p>
                                            </div>
                                        </div>
                                        <p className="text-xs sm:text-base font-medium text-white/90 leading-relaxed italic">
                                            "{result.reasoning}"
                                        </p>
                                    </div>
                                </motion.div>
                            ...
                            {status === 'idle' && (
                                ...
                                >
                                    <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mx-auto opacity-20">
                                        <Orbit className="h-8 sm:h-10 text-muted-foreground" />
                                    </div>
