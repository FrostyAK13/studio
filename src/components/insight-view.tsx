...
export function InsightView({ price, decimalPlaces, lastDigitTicks, priceHistory, selectedMarket, onMarketChange, maxTicks }: InsightViewProps) {
    ...
    const DirectionalIcon = ({ direction }: { direction: string }) => {
        ...
        if (isUp) return <TrendingUp className="h-5 w-5 sm:h-7 sm:w-7 text-emerald-600" />;
        if (isDown) return <TrendingDown className="h-5 w-5 sm:h-7 sm:w-7 text-rose-600" />;
        return <Crosshair className="h-5 w-5 sm:h-7 sm:w-7 text-primary" />;
    };
    ...
    const ProtocolCard = ({ insight }: { insight: ProtocolInsight }) => {
        ...
        return (
            <div className="p-4 sm:p-6 bg-white rounded-2xl sm:rounded-[2rem] border border-slate-200 relative overflow-hidden group shadow-sm transition-all hover:bg-slate-50 hover:border-primary/20">
                ...
                <div className="flex items-center justify-between mb-2 sm:mb-4">
                    ...
                    <Badge className={cn(
                        ...
                        riskLevel === 'STABLE' ? "bg-emerald-100 text-emerald-700" : riskLevel === 'LOW' ? "bg-amber-100 text-amber-700" : "bg-rose-100 text-rose-700"
                    )}>
                        {riskLevel}
                    </Badge>
                </div>
                ...
                <div className="space-y-3 sm:space-y-4">
                    <div>
                        <div className="flex items-center gap-2 sm:gap-3 mb-1">
                            <span className="text-base sm:text-2xl font-black text-slate-950 tracking-tighter">{insight.direction}</span>
                            {isPositive ? <ArrowUp className="h-3 w-3 sm:h-5 sm:w-5 text-emerald-600 animate-bounce" /> : <ArrowDown className="h-3 w-3 sm:h-5 sm:w-5 text-rose-600 animate-bounce" />}
                        </div>
                        <p className="text-[7px] sm:text-[9px] font-black text-slate-400 uppercase tracking-wider">{insight.summary}</p>
                    </div>
                    ...
                    <div className="space-y-1 sm:space-y-2">
                        <div className="flex justify-between items-end">
                             <div className="flex items-baseline gap-1.5">
                                ...
                                <p className="text-[6px] sm:text-[8px] font-black text-slate-400 uppercase tracking-widest">STABILITY</p>
                             </div>
                             <Gauge className="h-3 w-3 sm:h-4 sm:w-4 text-emerald-600/40" />
                        </div>
                        <Progress value={insight.confidence} className="h-1 sm:h-1.5 bg-slate-100 [&>div]:bg-emerald-500" />
                    </div>

                    <div className="pt-2 sm:pt-4 border-t border-slate-100">
                        <p className="text-[9px] sm:text-[12px] font-medium text-slate-600 leading-relaxed italic border-l-2 border-primary/40 pl-2">
                            "{insight.reasoning}"
                        </p>
                    </div>
                </div>
                 ...
                 return (
                    <HackerAnimation title={`INTELLIGENCE REPORT: ${marketName.toUpperCase()}`}>
                        {analysisState === 'analyzing' ? (
                            ...
                            <div className="text-left text-rose-600 flex items-start gap-3 p-4 sm:p-6 bg-rose-50 rounded-2xl sm:rounded-[2rem] border border-rose-200">
                            ...
                            <div className="text-left space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-5 duration-700">
                                <div className="p-4 sm:p-6 bg-slate-50 rounded-2xl sm:rounded-[2rem] border border-slate-200 shadow-inner relative overflow-hidden group">
                                    ...
                                    <p className="text-[11px] sm:text-base font-medium text-slate-900 leading-relaxed italic px-1">"{multiInsight.globalSummary}"</p>
                                </div>
    ...
    return (
        <div className="space-y-6 sm:space-y-10 animate-in fade-in slide-in-from-bottom-10 duration-700 pb-24 overflow-hidden">
             <Card className="border-none shadow-sm bg-white overflow-hidden relative rounded-2xl sm:rounded-[2.5rem] border border-slate-200">
                ...
                <CardContent className="p-5 sm:p-10 grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-12 items-center">
                    <div className="space-y-2 sm:space-y-3">
                        ...
                        <Select value={selectedMarket} onValueChange={onMarketChange}>
                            <SelectTrigger className="h-10 sm:h-14 bg-slate-50 border-slate-200 rounded-xl sm:rounded-2xl font-black text-xs sm:text-base px-4">
                                <SelectValue placeholder="Select Index" />
                            </SelectTrigger>
                            <SelectContent side="bottom" position="popper" sideOffset={8} className="w-[var(--radix-select-trigger-width)] max-h-[300px] rounded-xl sm:rounded-2xl border-slate-200 bg-white text-slate-950 z-[100] shadow-2xl">
                                {syntheticIndices.map((index) => (
                                <SelectItem key={index.id} value={index.id} className="focus:bg-primary/10 focus:text-slate-950 cursor-pointer py-2.5 px-4 font-black text-[11px] sm:text-xs">
                                    {index.name}
                                </SelectItem>
                                ))}
                            </SelectContent>
                    ...
                    <div className="space-y-2 sm:space-y-3">
                        ...
                        <div className="relative">
                            <div className="h-10 sm:h-14 bg-slate-50 border-slate-200 rounded-xl sm:rounded-2xl flex items-center justify-center font-black text-base sm:text-xl text-primary shadow-inner">
            ...
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-6 justify-between px-4 sm:px-6">
                    <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 text-center sm:text-left">
                        <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-primary/10 flex items-center justify-center border border-primary/20 shadow-sm">
                            <Bot className="h-5 w-5 sm:h-7 sm:w-7 text-primary" />
                        </div>
                        <div>
                            <h3 className="text-sm sm:text-xl font-black uppercase tracking-widest text-slate-900 leading-tight">STRATEGY INTELLIGENCE</h3>
                        </div>
                    </div>
                    <div className="text-center sm:text-right bg-white px-4 sm:px-6 py-1.5 sm:py-3 rounded-xl sm:rounded-2xl border border-slate-200 shadow-sm w-full sm:w-auto">
                         <p className="text-[7px] sm:text-[9px] font-black uppercase text-slate-400 tracking-widest mb-0.5">LIVE PIVOT</p>
                         <p className="text-base sm:text-2xl font-black text-slate-950 tabular-nums tracking-tighter">{price.toFixed(decimalPlaces)}</p>
                    </div>
                </div>
                    ...
                    <div className="text-center py-4">
                        <Button 
                            ...
                            className={cn(
                                ...
                                analysisState === 'analyzing' ? "bg-slate-200 text-slate-500 cursor-not-allowed" : "bg-primary text-primary-foreground hover:bg-primary/90"
                            )}
                        >
