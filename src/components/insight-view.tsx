...
export function InsightView({ globalResults, activeScanId, dashboardPrice, dashboardMarketId }: InsightViewProps) {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-1000 pb-24 max-w-[1600px] mx-auto px-2">
            <Card className="border-none shadow-2xl bg-slate-900/40 backdrop-blur-3xl rounded-3xl border border-white/5 overflow-hidden relative">
                ...
                <CardHeader className="pb-4 pt-10 px-8 border-b border-white/5 bg-black/40">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <CardTitle className="text-2xl font-black text-white uppercase leading-none tracking-tighter">PROBABILITY FLOW</CardTitle>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                                    <div className="lg:col-span-7 space-y-8">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                            <div className="p-8 rounded-[2rem] bg-black/60 border border-white/5 shadow-2xl relative overflow-hidden">
                                                <div className="absolute top-0 left-0 w-1 h-full bg-primary" />
                                                <p className="text-[10px] font-black text-primary uppercase tracking-[0.4em] mb-4">TRIGGER [e]</p>
                                                <div className="flex items-baseline gap-4">
                                                    <span className={cn("text-7xl font-black tabular-nums transition-all", triggerDetected ? "text-emerald-400 scale-110" : "text-white")}>{lockedSignal.triggerDigit}</span>
                                                </div>
                                            </div>
                                            <div className="p-8 rounded-[2rem] bg-black/60 border border-white/5 shadow-2xl relative overflow-hidden">
                                                <div className="absolute top-0 left-0 w-1 h-full bg-emerald-500" />
                                                <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.4em] mb-4">TARGET [t]</p>
                                                <div className="flex items-baseline gap-4">
                                                    <span className={cn("text-7xl font-black tabular-nums transition-all", targetHit ? "text-emerald-400 scale-125" : "text-white")}>{lockedSignal.targetDigit}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div className={cn("p-8 rounded-[2rem] border shadow-2xl transition-all duration-500", triggerDetected ? "bg-emerald-500/20 border-emerald-500 shadow-emerald-500/20" : "bg-white/5 border-white/5")}>
                                            <div className="flex items-center justify-between mb-8">
                                                <div className="flex items-center gap-6">
                                                    <div className={cn("p-4 rounded-xl", triggerDetected ? "bg-emerald-500" : "bg-white/10")}>
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <p className="text-xl font-black text-white tabular-nums tracking-tighter">{(livePrice || lockedSignal.currentPrice).toFixed(livePip || lockedSignal.pip)}</p>
                                                </div>
                                            </div>
                                            <div className="space-y-4">
                                                ...
                                                <div className="flex gap-2.5 justify-start overflow-hidden h-14 items-center">
                                                    {liveDigits.map((digit, idx) => {
                                                        ...
                                                        return (
                                                            <div key={`${idx}-${digit}`} className={cn("w-12 h-12 rounded-xl flex items-center justify-center font-black text-sm border transition-all shrink-0", isTrigger ? "bg-primary border-primary text-white scale-110 z-10" : isFlowSuccess ? "bg-emerald-500 border-emerald-500 text-white scale-125 z-20" : "bg-white/5 border-white/10 text-white/10")}>{digit}</div>
                                                        );
                                                    })}
                                                </div>
                                            </div>
                                    ...
                                    <div className="lg:col-span-5 space-y-6">
                                        <Card className="bg-black/40 border-white/5 p-8 rounded-[2rem] space-y-8">
                                            ...
                                            <div className="p-6 bg-white/5 rounded-3xl border border-white/5">
                                                <p className="text-[10px] font-bold text-white/80 leading-relaxed italic uppercase">"TRIGGER ON <span className="text-primary">{lockedSignal.triggerDigit}</span>, TRADE TOWARD <span className="text-emerald-500">{lockedSignal.targetDigit}</span> ON NEXT TICK."</p>
                                            </div>
                                        </Card>
