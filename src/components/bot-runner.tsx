...
export function BotRunner({ price, lastDigitTicks, selectedMarket, decimalPlaces }: BotRunnerProps) {
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 h-full pb-20">
            <Card className="xl:col-span-1 border-none shadow-2xl bg-slate-950/90 backdrop-blur-2xl overflow-hidden relative border border-white/5 animate-in slide-in-from-left-4 duration-500 rounded-[1.5rem] sm:rounded-[2.5rem]">
                ...
                <CardHeader className="pb-4 pt-6 px-6 space-y-6">
                    <div className="text-center">
                        ...
                        <p className="text-3xl sm:text-4xl font-black text-white tabular-nums">${balance.toFixed(2)}</p>
                    </div>
                    <Button 
            ...
            <Card className="xl:col-span-3 border-none shadow-2xl bg-slate-950/90 backdrop-blur-2xl overflow-hidden relative border border-white/5 rounded-[1.5rem] sm:rounded-[2.5rem] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                    {trades.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {trades.map((trade) => (
                                <div key={trade.id} className="grid grid-cols-6 items-center p-4 bg-black/40 border border-white/5 rounded-2xl transition-all hover:bg-black/60">
                                    <div className="col-span-1">
                                        <p className="text-[8px] font-black text-muted-foreground uppercase mb-0.5">TIME</p>
                                        <p className="text-xs font-black text-white">{trade.time}</p>
                                    </div>
                                    <div className="col-span-1">
                                        <p className="text-[8px] font-black text-muted-foreground uppercase mb-0.5">STRATEGY</p>
                                        <Badge className={cn("text-[8px] font-black uppercase border-none", trade.type === 'OVER' ? "bg-cyan-500/10 text-cyan-400" : "bg-emerald-500/10 text-emerald-400")}>
                                            {trade.type} {trade.barrier}
                                        </Badge>
                                    </div>
                                    <div className="col-span-1 text-center">
                                        <p className="text-[8px] font-black text-muted-foreground uppercase mb-0.5">TRIGGER</p>
                                        <p className="text-sm font-black text-white">{trade.entry}</p>
                                    </div>
                                    <div className="col-span-1 text-center">
                                        <p className="text-[8px] font-black text-muted-foreground uppercase mb-0.5">EXIT</p>
                                        <p className="text-sm font-black text-white">{trade.exit}</p>
                                    </div>
                                    <div className="col-span-1 text-center">
                                        <p className="text-[8px] font-black text-muted-foreground uppercase mb-0.5">RESULT</p>
                                        <p className={cn("text-xs font-black uppercase", trade.result === 'WON' ? "text-emerald-400" : "text-rose-500")}>
                                            {trade.result}
                                        </p>
                                    </div>
